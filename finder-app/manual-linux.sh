...
# echo ${SYS_ROOT}
cp ${SYS_ROOT}/lib/ld-linux-aarch64.so.1 lib/ld-linux-aarch64.so.1
cp ${SYS_ROOT}/lib64/libm.so.6 lib64/libm.so.6
cp ${SYS_ROOT}/lib64/libresolv.so.2 lib64/libresolv.so.2
...
# mkimage -A arm64 -O linux -T kernel -d  ${OUTDIR}/initramfs.cpio.gz ${OUTDIR}/Image
